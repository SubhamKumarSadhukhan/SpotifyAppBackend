...
async function initialize() {
  ...
  await db.none(
    "CREATE OR REPLACE VIEW topsongs as select DISTINCT i.name,i.rate,i.date_of_release,i.cover,(select array(select name from (select * from song where song_id =i.id) as k inner join artists on artist_id=artists.id)) artists from (SELECT songs.id,name,songs.date_of_release,songs.cover,coalesce(round(avg(rate)),0) as rate from songs_ratings right join songs on song_id=songs.id group by songs.id,songs.name order by rate desc limit 10) AS i left join song on i.id=song.song_id order by rate desc"
  );
  await db.none(
